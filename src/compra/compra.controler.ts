...
async function getAll(req: Request, res: Response) {
  try {
    const compras = await em.find(
      Compra,
      {},
      { populate: ['persona', 'items', 'items.producto'] }
    );
    return res.status(200).json({ message: 'Compras finded', data: compras });
  } catch (erro: any) {
    console.log(erro);
    return res.status(404).json({ message: 'Compras not finded' });
  }
}
  ...
  try {
    const compras = await em.find(
      ...
      { persona: personaId },
      {
        populate: [
          'items.producto.persona.direccion.localidad',
          'items.persona.direccion.localidad',
          'items.seguimiento.estados',
          'direccion.localidad',
        ],
      }
    ...
    if (comprasConItems.length === 0) {
      return res
        .status(404)
        .json({ message: 'No se encontraron compras con productos' });
    }
...
async function remove(req: Request, res: Response) {
  try {
    ...
    const compra = await em.findOneOrFail(
      Compra,
      { id },
      { populate: ['items.seguimiento.estados'] }
    );
    let remove = true;
    ...
    if (remove) {
      ...
      return res.status(400).json({
        message:
          'La compra no puede ser eliminado, tiene items que aun no llegaron',
      });
    }
  } catch (error: any) {
...
async function updateStock(req: Request, res: Response) {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ message: 'Error interno al actualizar stock' });
  }
}
...
async function getVentasByUser(req: Request, res: Response) {
  try {
    ...
    return res
      .status(500)
      .json({ message: 'Error al obtener ventas del usuario' });
  }
}
