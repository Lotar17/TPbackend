...
async function getAll(req: Request, res: Response) {
  try{ 
    const persona = await em.find(Persona,{},{populate:['prods_publicados']})

    return res.status(200).json({message:'found all persons',data:persona})

  }
  catch(error: any){return res.status(500).json({message : error.data})}
}

async function getOne(req: Request, res: Response) {
  try{
    const id = req.params.id
    const persona = await em.findOneOrFail(Persona,{id},{populate:['prods_publicados']})
...
async function add(req: Request, res: Response) {
  try{
    ...
    await em.flush()
    return res.status(200).json({message:'Person Created succesfully !',data:persona})
  }
  catch(error:any){
    return res.status(500).json({message : error.data})
  }
...
async function update(req: Request, res: Response) {
  try{
    ...
    await em.flush()
    return res.status(200).json({message:'Person updated succesfully !', data:personaToUpdate})
  }
  catch(error:any){
...
async function remove(req: Request, res: Response) {
  try{
    ...
    return res.status(200).json({message:'Person deleted succesfully !',data:persona})
  }
  catch(error:any){
    return res.status(500).json({message : error.data})
  }
